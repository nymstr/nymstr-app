...
pub struct KeyPackageManager {
    ...
    credential_cache: std::sync::Arc<std::sync::Mutex<HashMap<String, MlsCredential>>>,
    db: std::sync::Arc<Db>,
}
...
impl KeyPackageManager {
    pub fn new(db: std::sync::Arc<Db>) -> Self {
        Self {
            ...
            credential_cache: std::sync::Arc::new(std::sync::Mutex::new(HashMap::new())),
            db,
        }
    }
    ...
    ///
    /// # Arguments
    /// * `mls_client` - The MLS client to generate the key package with
    /// * `credential` - The MLS credential binding the PGP identity to the MLS key
    ///
    /// # Returns
    /// A base64-encoded key package string along with the credential
    ...
    pub fn validate_key_package(&self, key_package_b64: &str) -> Result<bool> {
        ...
        // Check for expiry if lifetime extension is present
        // Note: MLS key packages have a lifetime extension that we should validate
        // The exact API depends on mls-rs version
    ...
    ///
    /// This method performs comprehensive validation of a key package including
    /// verification of the associated credential binding.
    ///
    /// # Arguments
    /// * `key_package_b64` - Base64-encoded key package
    /// * `credential` - The MLS credential to verify against
    /// * `pgp_public_key_bytes` - The PGP public key bytes for binding verification
    ///
    /// # Returns
    /// Detailed validation result including credential verification status
