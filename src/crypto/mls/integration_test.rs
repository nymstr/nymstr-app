...
mod tests {
    ...
    async fn test_full_mls_conversation_flow() {
        ...
        let welcome_message = conversation_info
            .welcome_message
        ...
        let encrypted = alice
            .encrypt_message(&conversation_id, test_message)
            .await
            .unwrap();
        ...
        assert!(
            !encrypted.mls_message.is_empty(),
        ...
        let bob_encrypted = bob
            .encrypt_message(&conversation_id, bob_message)
            .await
            .unwrap();
    ...
    async fn test_mls_group_member_addition() {
        ...
        let encrypted = alice
            .encrypt_message(&conversation_id, initial_msg)
            .await
            .unwrap();
        let decrypted = bob.decrypt_message(&encrypted).await.unwrap();
        ...
        let add_commit = alice
            .add_member(&conversation_id, &charlie_key_package)
            .await
            .unwrap();

        // Verify the commit message is produced
        assert!(
            !add_commit.mls_message.is_empty(),
            "Add member commit should produce a message"
        );
        ...
        let encrypted_post = alice
            .encrypt_message(&conversation_id, post_add_msg)
            .await
            .unwrap();
    ...
    async fn test_mls_message_ordering() {
        ...
        for (i, msg) in messages.iter().enumerate() {
            ...
            for prev_encrypted in &encrypted_messages {
                assert_ne!(
                    encrypted.mls_message, *prev_encrypted,
                    "Message {} should be uniquely encrypted",
                    i
                );
            }
    ...
    async fn test_mls_group_state_consistency() {
        ...
        for i in 0..5 {
            ...
            let encrypted = alice
                .encrypt_message(&conversation_id, msg.as_bytes())
                .await
                .unwrap();
            let decrypted = bob.decrypt_message(&encrypted).await.unwrap();
            ...
            let encrypted_reply = bob
                .encrypt_message(&conversation_id, reply.as_bytes())
                .await
                .unwrap();
        ...
        assert!(
            !alice_state.is_empty(),
            "Alice should have exportable group state"
        );
        assert!(
            !bob_state.is_empty(),
            "Bob should have exportable group state"
        );
    }
    ...
    async fn test_mls_error_conditions() {
        ...
        assert!(
            ...
            "Should fail to encrypt to non-existent group"
        );
