...
impl MessageHandler {
    ...
    pub async fn authenticate_group(&mut self, username: &str, group_server_address: &str) -> anyhow::Result<bool> {
        ...
        let sign_content = format!("register:{}:{}:{}", username, group_server_address, timestamp);
        let signature = Crypto::pgp_sign_detached_secure(secret_key, sign_content.as_bytes(), &passphrase)?;

        self.service.register_with_group_server(
            username,
            &public_key_armored,
            &signature,
            timestamp,
    ...
    pub async fn send_group_message(&mut self, message: &str, group_server_address: &str) -> anyhow::Result<()> {
        ...
        let sender = self.current_user.as_deref()
        ...
        let mls_client = MlsClient::new(sender, Arc::clone(&secret_key), Arc::clone(&public_key), self.db.clone(), &passphrase)?;
        ...
        let encrypted = mls_client.encrypt_message(&conversation_id, message.as_bytes()).await?;
        ...
        let signature = crate::crypto::Crypto::pgp_sign_detached_secure(&secret_key, ciphertext.as_bytes(), &passphrase)?;

        // Send the encrypted message to the group server
        self.service.send_group_message(sender, &ciphertext, &signature, group_server_address).await?;
    ...
    pub async fn fetch_group_messages(&mut self, group_server_address: &str) -> anyhow::Result<()> {
        ...
        let signature = if let (Some(secret_key), Some(passphrase)) = (&self.pgp_secret_key, &self.pgp_passphrase) {
            crate::crypto::Crypto::pgp_sign_detached_secure(secret_key, last_seen_id.to_string().as_bytes(), passphrase)?
        } else {
            return Err(anyhow::anyhow!("PGP keys not available for signing"));
        };

        // Send fetch request
        self.service.send_group_fetch_request(user, last_seen_id, &signature, group_server_address).await?;
        log::info!("Sent fetchGroup request to {} with cursor {}", group_server_address, last_seen_id);
    ...
    async fn try_decrypt_with_any_group(
        ...
        for group_id in groups {
            match mls_manager.process_incoming_message_buffered(group_id, sender, ciphertext).await {
                Ok(Some((_, text))) => {
                    return Ok(Some((sender.to_string(), text)));
                }
                Ok(None) => {
                    // Message was buffered for later - epoch mismatch
                    log::info!("Group message from {} buffered for group {} (epoch mismatch)", sender, group_id);
        ...
        match action {
            "fetchGroupResponse" => {
                ...
                let content_json: serde_json::Value = serde_json::from_str(content)
                    .map_err(|e| anyhow::anyhow!("Failed to parse fetchGroupResponse content: {}", e))?;

                let messages = content_json.get("messages")
                    .and_then(|v| v.as_array())
                    .ok_or_else(|| anyhow::anyhow!("Missing messages array in fetchGroupResponse"))?;
                ...
                for msg in messages {
                    ...
                    match Self::try_decrypt_with_any_group(mls_manager, &groups, sender, ciphertext).await {
                        Ok(Some((sender, text))) => {
                        ...
                        Err(()) => {
                            log::warn!("Could not decrypt message from {} with any known group", sender);
                        }
                    }
                }
            ...
            "registerResponse" => {
                ...
                if content == "pending" {
                    Ok(vec![("SYSTEM".to_string(), "Registration pending admin approval".to_string())])
                } else if content.starts_with("error:") {
                    Ok(vec![("SYSTEM".to_string(), format!("Registration failed: {}", content))])
                } else {
                    Ok(vec![])
                }
            }
